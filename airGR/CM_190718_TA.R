...
#setwd("~/Dropbox/School Notes Laptop/Fall 2018/Github_Sesync/Raven_GR4J/airGR")
setwd("/nfs/HealthandResourcePolicy-data/Task 2/Tamee_modeling/handouts_sesync/airGR")
#these are functions written elsewhere. RDA files are direct equations (alt to csv but loads more quickly, e.g. output of a linear model that was run)
source("stage_vol.R")
source("stage_area.R")
source("vol_stage.R")
source("Q_max.R")
...
points(stage[1:365]) #object stage not found
...
for (i in 1:length(precip)){
  ...
  Qmax2[i] = cur_max #cur max is what the stage is expected to be on that day
  
  #Select only the rainy season for modified WL fluctuations
  daily = 1:365
  rainy = daily[-(32:243)]
  int = 7 #interval of drop sequence, in days
  n <- length(rainy)
  r_int <- rainy[seq(1,n,int)]
  ...
  for (i in r_int){
    ...
    release = i:i+(per-1) ###############
    for (i in release){  #here, I'm trying to say for the current day and the next 4 days, do this: 
      if (temp_stage > goal_stage){	
        cur_Q = (drop_day*cur_area)/24 #hourly rate of release
      } else if (cur_stage > cur_max){
...
#not sure how to connect this with previous
...
plot(stage_zoo[365:1826],type='l',ylab = 'Reservoir Water Level (m)', xlab = 'Years', main = 'Simulated Reservoir Water Level: 1996-2000')

HP_zoo = zoo(HP,as.Date(index(precip)))*24
HP_an = (daily2annual(HP_zoo[365:1826],FUN = sum)*0.001)
barplot(HP_an,year(HP_an),ylab = 'Annual HP Produced (GwH)', xlab = 'Years', main = ' Annual HP Production :1996-2000', ylim=c(0,1000))


par(mfrow=c(2,1))
#test = izoo2rzoo(stage_zoo,tstep = "days",from = "1996-05-01",to="2000-04-30")
stage_mon = daily2monthly(stage_zoo[365:1826], FUN = mean)
