...
for (i in 1:365){
  ...
  #m3/sec to km3/day
  cur_inflow = (coredata(inflow[i])*24*60*60)/(10^9)
  ...
  } else if (cur_stage < cur_min){
    cur_Q = 0
  } else if (cur_stage < 203){
    temp_x = data.frame(H=cur_stage)
    cur_H = predict(f,temp_x)
    cur_Q = predict(f2,temp_x)
  } else if (cur_stage >= 203 & cur_stage<208){
    temp_x = data.frame(H=cur_stage)
    cur_H = 205
    cur_Q = predict(f2,temp_x)
  ...
  vol[i] = cur_vol + precip_vol - ET_vol - Q_vol[i]
