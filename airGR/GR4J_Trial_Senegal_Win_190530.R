...
PE_data = PEdaily_Oudin(jd, input$Temperature, Lat = 13, LatUnit = "deg")
...
flow_mmday = flow_mmday2
 flow_mmday[(flow_mmday < quantile(flow_mmday,0.1,na.rm = TRUE))|
              (flow_mmday  > quantile(flow_mmday,0.95,na.rm = TRUE))] = NA
#flow_mmday[(flow_mmday > quantile(flow_mmday,0.9,na.rm = TRUE))] = NA
...
InputsCrit <- CreateInputsCrit(FUN_CRIT = ErrorCrit_NSE, InputsModel = InputsModel, 
                               RunOptions = RunOptions, Obs = flow_mmday[Ind_Run])
str(InputsCrit)
...
plot(OutputsModel, Qobs = flow_mmday[Ind_Run])
