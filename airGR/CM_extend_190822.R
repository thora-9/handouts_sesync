setwd('C:/Users/tejasvi/Dropbox/School Notes/Spring 2019/SESYNC_Modelling/handouts_sesync/airGR')
...
precip = ppt_temp2$trmm_zoo


#Currently directly plotting the precip data gives errors; use core data to plot
inflow = zoo(OutputsModel$Qsim, dates4[Ind_Run])*(27800/10^6) #Units km3/day
...
for (i in 1:length(inflow)){
  ...
  #m3/sec to km3/day
  cur_inflow = coredata(inflow[i])#(coredata(inflow[i])*24*60*60)/(10^9) 
...
#Variable
stage_zoo = zoo(stage,as.Date(index(inflow)))
plot(stage_zoo[365:length(inflow)],type='l',ylab = 'Reservoir Water Level (m)', xlab = 'Years', main = 'Simulated Reservoir Water Level: 1996-2000')
